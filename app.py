...
def safe_trash(path):
    """Try send2trash first, fallback to os.remove"""
    try:
...
def stats():
    ...
    ss_count = len([f for f in os.listdir(SCREENSHOT_FOLDER) if os.path.isfile(os.path.join(SCREENSHOT_FOLDER, f))]) if os.path.exists(SCREENSHOT_FOLDER) else 0
    ...
    procs = sorted(procs, key=lambda x: float(x["ram"].replace(" MB","").replace(" GB","")), reverse=True)[:8]
    health = max(0, 100 - int(disk.percent*0.4) - int(mem.percent*0.3) - int(cpu*0.3))
    return jsonify({
        "cpu": round(cpu,1), "ram_percent": round(mem.percent,1),
...
def clean_screenshots():
    if not os.path.exists(SCREENSHOT_FOLDER):
        return jsonify({"ok": False, "msg": f"Screenshot folder not found: {SCREENSHOT_FOLDER}"})
    ...
    cleaned, failed = 0, 0
    for f in files:
        if safe_trash(os.path.join(SCREENSHOT_FOLDER, f)):
            cleaned += 1
        else:
            failed += 1
    log_action(f"Cleaned {cleaned} screenshots")
    msg = f"✅ {cleaned} screenshots moved to Recycle Bin."
    if failed: msg += f" ({failed} failed)"
    return jsonify({"ok": True, "msg": msg, "count": cleaned, "failed": failed})

@app.route("/api/clean/recyclebin", methods=["POST"])
def empty_recycle_bin():
    try:
        # Use PowerShell — works without winshell admin issues
        ...
        # Fallback: try winshell
        try:
            import winshell
            winshell.recycle_bin().empty(confirm=False, show_progress=False, sound=False)
            log_action("Recycle Bin emptied (winshell)")
            return jsonify({"ok": True, "msg": "✅ Recycle Bin emptied successfully."})
        except Exception as e2:
            return jsonify({"ok": False, "msg": f"Failed: {str(e2)}"})
...
def clean_downloads():
    ...
    junk = [f for f in os.listdir(DOWNLOAD_FOLDER) if any(f.lower().endswith(e) for e in junk_ext)]
    if not junk:
        return jsonify({"ok": True, "msg": "No junk files found — Downloads is clean!", "count": 0})
    cleaned, failed = 0, 0
    for f in junk:
        if safe_trash(os.path.join(DOWNLOAD_FOLDER, f)):
            cleaned += 1
        else:
            failed += 1
    log_action(f"Cleaned {cleaned} junk files from Downloads")
    return jsonify({"ok": True, "msg": f"✅ {cleaned} junk files cleaned.", "count": cleaned})
...
def full_clean():
    ...
    if os.path.exists(SCREENSHOT_FOLDER):
        files = [f for f in os.listdir(SCREENSHOT_FOLDER) if os.path.isfile(os.path.join(SCREENSHOT_FOLDER, f))]
        cleaned = sum(1 for f in files if safe_trash(os.path.join(SCREENSHOT_FOLDER, f)))
        if cleaned: results.append(f"{cleaned} screenshots cleaned")
    if os.path.exists(DOWNLOAD_FOLDER):
        junk = [f for f in os.listdir(DOWNLOAD_FOLDER) if any(f.lower().endswith(e) for e in [".tmp",".log",".part",".cache",".crdownload"])]
        cleaned = sum(1 for f in junk if safe_trash(os.path.join(DOWNLOAD_FOLDER, f)))
        if cleaned: results.append(f"{cleaned} junk files cleaned")
    try:
...
def organise_desktop():
    ...
    files = [f for f in os.listdir(DESKTOP_FOLDER)
             if os.path.isfile(os.path.join(DESKTOP_FOLDER, f)) and not f.lower().endswith((".lnk",".url"))]
    if not files:
        return jsonify({"ok": True, "msg": "Desktop is already clean!", "count": 0, "breakdown": {}})
    moved = 0
    breakdown = {}
    ...
    return jsonify({
        "ok": True,
        "msg": f"✅ {moved} files organised into Desktop/Organised/ subfolders.",
        "count": moved,
        "breakdown": breakdown,
        "dest": dest_base,
        "skipped": len(skipped)
...
@app.route("/api/organise/downloads", methods=["POST"])
def organise_downloads():
    if not os.path.exists(DOWNLOAD_FOLDER):
        return jsonify({"ok": False, "msg": "Downloads folder not found."})
    files = [f for f in os.listdir(DOWNLOAD_FOLDER) if os.path.isfile(os.path.join(DOWNLOAD_FOLDER, f))]
    if not files:
        return jsonify({"ok": True, "msg": "Downloads is empty!", "count": 0, "breakdown": {}})
    moved = 0
    breakdown = {}
    dest_base = os.path.join(DOWNLOAD_FOLDER, "Organised")
    for f in files:
        ext = os.path.splitext(f)[1].lower()
        cat = next((c for c, exts in FILE_CATEGORIES.items() if ext in exts), "Others")
        dest = os.path.join(dest_base, cat)
    ...
    log_action(f"Organised {moved} files in Downloads")
    return jsonify({
        "ok": True,
        "msg": f"✅ {moved} files organised into Downloads/Organised/ subfolders.",
        "count": moved,
        "breakdown": breakdown,
        "dest": dest_base
    })
...
def find_large_files():
    ...
    return jsonify({"ok": True, "files": large[:30], "total": len(large)})
...
def delete_file():
    ...
    try:
        ...
        safe_trash(fp)
        log_action(f"Deleted: {name}")
        return jsonify({"ok": True, "msg": f"✅ {name} moved to Recycle Bin."})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)})

@app.route("/api/files/preview")
def preview_folder():
    folder = request.args.get("folder", DESKTOP_FOLDER)
    ...
    for f in os.listdir(folder):
        ...
        try:
            items.append({
                ...
                "size_mb": round(os.path.getsize(fp)/(1024*1024),1) if os.path.isfile(fp) else None
            })
        except: pass
    return jsonify({"ok": True, "files": items, "path": folder})
...
def get_processes():
    ...
    procs = sorted(procs, key=lambda x: x["ram_mb"], reverse=True)[:20]
...
def kill_process():
    ...
    pid = data.get("pid")
    if not pid:
        return jsonify({"ok": False, "msg": "No PID provided"})
    try:
        p = psutil.Process(pid)
        name = p.name()
        p.kill()
        log_action(f"Killed process: {name} (PID {pid})")
        return jsonify({"ok": True, "msg": f"✅ {name} (PID {pid}) terminated."})
    except psutil.AccessDenied:
        return jsonify({"ok": False, "msg": "Access denied — run as Administrator."})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)})
...
def ai_analyse():
    ...
    mem  = psutil.virtual_memory()
    disk = psutil.disk_usage("C:\\")
    ss   = len([f for f in os.listdir(SCREENSHOT_FOLDER) if os.path.isfile(os.path.join(SCREENSHOT_FOLDER, f))]) if os.path.exists(SCREENSHOT_FOLDER) else 0
...
@app.route("/api/logs")
def get_logs():
    return jsonify({"logs": action_logs[:30], "total": len(action_logs)})
