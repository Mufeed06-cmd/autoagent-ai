...
SCREENSHOT_FOLDER = f"C:\\Users\\{USERNAME}\\Pictures\\Screenshots"
DOWNLOAD_FOLDER   = f"C:\\Users\\{USERNAME}\\Downloads"
DESKTOP_FOLDER    = f"C:\\Users\\{USERNAME}\\Desktop"
AI_MODEL          = "qwen3:8b"
...
def stats():
    ...
    procs = sorted(procs, key=lambda x: float(x["ram"].replace(" MB","").replace(" GB","")
                   ), reverse=True)[:5]
    ...
    return jsonify({
        ...
        "ss_mb"       : ss_mb,
        "health"      : health,
        "uptime"      : boot.strftime("%Y-%m-%d %H:%M"),
        "processes"   : procs,
...
def clean_downloads():
    ...
    junk = [f for f in os.listdir(DOWNLOAD_FOLDER)
            if any(f.endswith(e) for e in junk_ext)]
    if not junk:
        return jsonify({"ok": True, "msg": "No junk files found.", "count": 0})
    for f in junk:
...
# ─── API: ORGANISE DESKTOP ────────────────────────────────
@app.route("/api/organise/desktop", methods=["POST"])
def organise_desktop():
    if not os.path.exists(DESKTOP_FOLDER):
        return jsonify({"ok": False, "msg": "Desktop folder not found."})
    files = [f for f in os.listdir(DESKTOP_FOLDER)
             if os.path.isfile(os.path.join(DESKTOP_FOLDER, f))
             and not f.endswith((".lnk",".url"))]
    if not files:
        return jsonify({"ok": True, "msg": "Nothing to organise.", "count": 0})
    moved = 0
...
def full_clean():
    ...
    if os.path.exists(DOWNLOAD_FOLDER):
        junk = [f for f in os.listdir(DOWNLOAD_FOLDER)
                if any(f.endswith(e) for e in [".tmp",".log",".part",".cache"])]
        for f in junk:
            send2trash.send2trash(os.path.join(DOWNLOAD_FOLDER, f))
...
Give me 3 quick bullet point recommendations. Be very concise."""
    ...
    try:
        response = ollama.chat(model=AI_MODEL,
